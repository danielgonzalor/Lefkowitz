...
import { FormuleLefkowitz, ResultFormuleLefkowitz } from "./interface/formule-lefkowitz.interface"
/**
 * 
 ...
 */
export const Lefkowitz = ({ lensMagnification, aperture, typeSensor, extension, focalDistance, overlap }: FormuleLefkowitz): ResultFormuleLefkowitz=>{ 
  let fEffective, depthOfFieldCentimeters;

  let realMagnification = calculateRealMagnificationFinite({ lensMagnification, extension, focalDistance })  
  fEffective = apertureEffectiveObjectiveMicroscope({ aperture, realMagnification })  
  depthOfFieldCentimeters = calculateDofCm({ typeSensor, realMagnification, overlap, fEffective })
